...
def transform_aabb(transform, aabb):
    ...
    It contains all corner points of the original AABB after applying the given transformation.
    ...
    #only h flip or rotate + flip,flip_x_chance=1  ########################################
    if transform[0, 0] == -1 or (transform [0,1] == -1 and transform [1,0] == -1):
         if ANY >0:
             ANY = 3.142-ANY
         else:
             ANY = -3.142-ANY

    # only v flip or rotate + flip
    if transform[1, 1] == -1 or (transform [0,1] == 1 and transform [1,0] == 1):
        ANY=-ANY

    #both h and v or rotate + flip
    if transform [0,1] == 1 and transform [1,0] == -1:
        if ANY > 0:
            ANY = 3.142 - ANY
        else:
            ANY = -3.142 - ANY
    ...
    # rotation:
    if transform[0, 1] != 0:
        if (ANY>=3.142/2 and ANY <= 3.142):  # 3
            ANY = ANY - 1.571*3
        else:   # 124
            ANY = ANY + 1.571

        h = max_corner[0]-min_corner[0]
        w = max_corner[1]-min_corner[1]
        shift = (w-h)/2
        return [min_corner[0]-shift, min_corner[1]+shift, max_corner[0]+shift, max_corner[1]-shift, ANY]
    else:
        return [min_corner[0], min_corner[1], max_corner[0], max_corner[1],ANY]
