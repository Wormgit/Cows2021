...
parser.add_argument("--print_id", default=0, type=int)
...
parser.add_argument("--ignore_clip_th_dt", default=0.9, type=float, help='')  #set to 0.1 nothing happen Percentage higher than it suvive
            ...
            if args.pre:
                ...
                for box, score, label in zip(boxes[0][indice], scores[0][indice], labels[0][indice]):
                    ...
                    if ignore_value == 0:
                        ...
                        c_lor = label_color(label)
                        #c_lor =  label_color(3)
                        draw_rbox(draw, box_pre[0], color=c_lor, thickness=8)  # 1 is light blue
                        # draw head
                        endx = int(xe + we / 1.5 * (math.cos(box[4])))
                        endy = int(ye + we / 1.5 * (math.sin(box[4])))
                        cv2.line(draw, (xe, ye), (endx, endy), color=c_lor, thickness=4)
