...
def trainFold(args):
	...
	for epoch in tqdm(range(args.num_epochs), desc="Training epochs"):
		...
		if epoch % args.eval_freq == 0:
			...
			utils.saveCheckpoint(epoch, model, optimiser, str(epoch)+"current")
...
if __name__ == '__main__':
	...
	parser.add_argument('--out_path', type=str, default="/home/will/Desktop/will_mine_155",
						help="Path to folder to store results in")

	# Fold settings
	parser.add_argument('--num_folds', type=int, default=10,
						help="Total of folds to cross validate across")
	parser.add_argument('--start_fold', type=int, default=6,
						help="The fold number to START at")
	parser.add_argument('--end_fold', type=int, default=6,
						help="The fold number to END at")

	# Dataset settings
	parser.add_argument('--dataset', type=str, default='RGBDCows2020',
						help='Which dataset to use: [RGBDCows2020]')
	parser.add_argument('--img_type', type=str, default='RGB',
						help='Which image type to retrieve from RGBDCows2020: [RGB]')
